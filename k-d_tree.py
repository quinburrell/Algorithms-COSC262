    ...
    def points_in_range(self, query_rectangle):
        ...
        if self.is_leaf:
            ...
            for point in self.points:
                if Vec(point[0], point[1]).in_box(query_rectangle[0], query_rectangle[1]):
                    temp += [point]
            return temp
        else:
            matches = []
            if self.axis == 0:
                if self.coord >= query_rectangle[0].x:
                    matches += self.leftorbottom.points_in_range(query_rectangle)
                if self.coord <= query_rectangle[1].x:
                    matches += self.rightortop.points_in_range(query_rectangle)
            else:
                if self.coord >= query_rectangle[0].y:
                    matches += self.leftorbottom.points_in_range(query_rectangle)
                if self.coord <= query_rectangle[1].y:
                    matches += self.rightortop.points_in_range(query_rectangle)                
        return matches
